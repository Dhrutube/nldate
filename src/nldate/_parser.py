...
_REL_FROM = re.compile(
    rf"^({_NUM})\s+(day|days|week|weeks|month|months|year|years)\s+from\s+(?:now|today)$"
)
...
def _try_before_after(s: str, ref: date) -> date | None:
    ...
    if not m:
        return None
    duration_part = s[: m.start()].replace(" and ", ", ")
    direction_word = m.group(1)
    anchor_part = s[m.end() :].strip()
    ...
    anchor = _try_keyword(anchor_part, ref)
    if anchor is None:
        anchor = _parse_absolute(anchor_part, ref)
    result = anchor
